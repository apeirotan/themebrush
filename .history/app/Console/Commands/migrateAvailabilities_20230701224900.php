...
class migrateAvailabilities extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'availabilities:migrate';
     ...
     * @var string
     */
    protected $description = 'Migrate availabilities';
    ...
    public function handle(): void
    {
        ...
        DB::connection('remote')->table('availability')->orderBy('name', 'asc')
        ->chunk(500, function ($availability) use ($time_start,  &$i) {
            $bar = $this->output->createProgressBar(count($availability));
            $data = [];
            $new_base_path = '';
